...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!text) {
      console.error('Unexpected response shape:', JSON.stringify(result.body));
    ...
    console.error('Keith HTTPS error:', err.message);
...
- Thank them warmly for precepting
- Include student name, school, program, rotation dates, hours required, student email, student phone
- Note student will reach out directly to coordinate schedules
- Remind them to attach: ASPIRE Brochure and Pre-licensure Student General Guidelines
- Mention preceptor pay (refer to Dr. Krystal Rodriguez), avoid being in charge while precepting, floating acceptable if comfortable
- Full signature below

EMAIL SIGNATURE (use in all drafted emails):
...
- Never fabricate data not in the live context below
- Draft emails only, never send automatically
- Refer to students by first and last name only
