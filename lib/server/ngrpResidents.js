...
import { narrowToStudents, demoScopeOf } from './demoScope.js'
...
export async function loadResidents(db, { cycleId = null, talentAcquisition = false } = {}) {
  ...
  // DEMO-MODE-2: a hire record is ABOUT a student, and the student is what carries
  // is_demo. ngrp_residency_outcomes does not, so the join is what decides whether this
  // row belongs to the population being presented.
  //
  // Without this the row does not disappear when its student is filtered away: it
  // renders with a BLANK NAME and every one of its own columns intact, which in the
  // Residents tab means a real Cedars-Sinai address, hire unit and start date on a demo
  // screen. The missing name is exactly what makes that easy to miss.
  //
  // In real mode this is a no-op by construction: the outcome table's FK to students is
  // RESTRICT, so a hired student cannot be deleted and is always in the fetch above.
  const inScope = narrowToStudents(
    outcomes,
    (students.data || []).map(s => s.id),
    demoScopeOf(db),
    (o) => o.student_id || candById.get(o.candidate_id)?.student_id)

  return {
    state: 'ok',
    detailsProvisioned,
    residents: composeResidents({
      outcomes: inScope, candidates: cands.data || [], students: students.data || [], cycles: cycles.data || [],
