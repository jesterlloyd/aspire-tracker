...
function styleBook() {
  ...
  return {
    ...
    of(f) {
      ...
      const xf = `<xf numFmtId="0" fontId="${fontId}" fillId="${fillId}" borderId="0" xfId="0" applyFont="1"${fillId ? ' applyFill="1"' : ''}${align ? ' applyAlignment="1"' : ''}>${align}</xf>`
...
export function xlsxFor({ sheetName = 'Responses', header, rows, widths: pxWidths = [] }) {
  ...
  const text = (c) => (c && typeof c === 'object' && 'v' in c ? c.v : c)
  ...
  rows.forEach((row, i) => {
    ...
    out.push(`<row r="${r}"${grouped ? ' outlineLevel="1"' : ''}>${row.map((v, c) => cell(r, c, text(v), book.of(v && typeof v === 'object' ? v.f : null))).join('')}</row>`)
  })
