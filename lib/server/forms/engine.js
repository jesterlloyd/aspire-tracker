...
import {
  definitionIssues, answerIssues, cleanAnswers, DEFAULT_SETTINGS, REMINDER_RULES, STARTER_FORMS, AUDIENCE_KEYS,
...
export async function installStarters(db, profile) {
  ...
  for (const s of STARTER_FORMS) {
    const { data: have } = await db.from('catalog_forms').select('id, status').eq('starter_key', s.slug).maybeSingle()
    if (have) { out.push({ key: s.slug, id: have.id, added: false }); continue }
