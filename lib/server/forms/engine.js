...
import {
  definitionIssues, answerIssues, cleanAnswers, DEFAULT_SETTINGS, REMINDER_RULES, STARTER_FORMS, RETIRED_STARTER_DRAFTS, AUDIENCE_KEYS,
...
// jsonb does not keep key order, so two definitions are compared with their keys sorted.
const stable = (v) => Array.isArray(v) ? v.map(stable)
  : v && typeof v === 'object' ? Object.fromEntries(Object.keys(v).sort().map(k => [k, stable(v[k])])) : v
const sameDefinition = (a, b) => JSON.stringify(stable(cleanDefinition(a))) === JSON.stringify(stable(cleanDefinition(b)))
...
export async function installStarters(db, profile) {
  ...
  for (const s of STARTER_FORMS) {
    ...
    if (have) {
      // A never-published draft still word for word as an earlier starter shipped it gets the
      // corrected starter; anything someone edited or published is left alone.
      const stale = have.status === 'draft' && (RETIRED_STARTER_DRAFTS[s.slug] || []).some(d => sameDefinition(d, have.draft))
      if (!stale) { out.push({ key: s.slug, id: have.id, added: false }); continue }
      await saveDraft(db, have.id, { draft: s.definition }, profile)
      if (s.publish) await publish(db, have.id, profile)
      out.push({ key: s.slug, id: have.id, added: false, refreshed: true, published: !!s.publish })
      continue
    }
...
export async function submit(db, { assignment, form, version, answers, ctx = {} }, { mailer, appUrl, timeZone = 'America/Los_Angeles' }) {
  ...
  try {
    pdf = await buildSubmissionPdf({ definition: def, answers: clean, who: { name: assignment.name, email: assignment.email },
      meta: { submittedAt, submissionId, version: assignment.form_version, timeZone } })
