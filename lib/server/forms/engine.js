...
import {
  definitionIssues, answerIssues, cleanAnswers, DEFAULT_SETTINGS, REMINDER_RULES, STARTER_FORMS, RETIRED_STARTER_DRAFTS, AUDIENCE_KEYS, sameDefinition, starterUpdateFor, sheetFor, summaryFor,
...
export async function sheetData(db, formId, { isDemo = false } = {}) {
  ...
  return { form, ...sheetFor(versions, rows), summary: summaryFor(versions, rows) }
}

/** The Sheet as an Excel workbook: the rows and columns the person is looking at, in that order. */
export async function sheetXlsx(db, formId, { rowIds = null, columnKeys = null, isDemo = false } = {}) {
  const { form, columns, rows } = await sheetData(db, formId, { isDemo })
  const byId = new Map(rows.map(r => [r.id, r]))
  const shownRows = Array.isArray(rowIds) ? rowIds.map(id => byId.get(id)).filter(Boolean) : rows
  const shownCols = Array.isArray(columnKeys) ? columns.filter(c => columnKeys.includes(c.key)) : columns
  ...
  const header = ['Name', 'Email', 'School', 'Submitted', 'Version', ...shownCols.map(c => (c.earlier ? `${c.label} (earlier version)` : c.label))]
  const body = shownRows.map(r => [r.name, r.email, r.school, stamp(r.submittedAt), String(r.version), ...shownCols.map(c => r.cells[c.key] || '')])
  const title = form.draft?.title || 'Form'
  return { bytes: xlsxFor({ sheetName: title, header, rows: body }), fileName: `${title.replace(/[^\w .-]+/g, '').slice(0, 80) || 'Form'} responses.xlsx` }
}
