 ...
 */
export async function sheetXlsx(db, formId, { rowIds = null, columnKeys = null, groupBy, isDemo = false } = {}) {
  ...
  // EXPORT-ONE-1: with no view from the Sheet (the header's Export to Excel on People or
  // Summary), the export follows the Sheet's SAVED layout: its column order, hidden columns
  // and grouping, over every answer. The Sheet screen orders the same way.
  const savedOrder = new Map((layout.order || []).map((k, i) => [k, i]))
  const hiddenKeys = new Set(layout.hidden || [])
  const inSavedLayout = all.map((c, i) => ({ c, at: savedOrder.has(c.key) ? savedOrder.get(c.key) : 1000 + i })).sort((a, b) => a.at - b.at).map(x => x.c).filter(c => !hiddenKeys.has(c.key))
  const shownCols = Array.isArray(columnKeys) ? columnKeys.map(k => all.find(c => c.key === k)).filter(Boolean) : inSavedLayout
  if (groupBy === undefined) groupBy = layout.groupBy || null
