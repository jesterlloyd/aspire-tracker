...
export const PAPER_FILES = { 'parking-spd': { sha256: PARKING_SPD_SHA256, name: 'Students Parking Data (SPD)' } }
...
export const paperLayoutOf = (form) => form?.starter_key === 'student-parking-request' ? 'parking-spd' : null
...
export async function paperStatus(db, form) {
  ...
  return { layout, name: PAPER_FILES[layout].name, onFile: !!(await paperOf(db, form)) }
}
