...
import {
  definitionIssues, answerIssues, cleanAnswers, DEFAULT_SETTINGS, REMINDER_RULES, STARTER_FORMS, RETIRED_STARTER_DRAFTS, AUDIENCE_KEYS, sameDefinition, starterUpdateFor, sheetFor, summaryFor, cleanLayout, cleanFormat, withCorrections, CORRECTABLE_TYPES, SHEET_FILLS, SHEET_INKS,
...
export async function sheetXlsx(db, formId, { rowIds = null, columnKeys = null, groupBy = null, isDemo = false } = {}) {
  ...
  const all = [{ key: '@school', label: 'School' }, { key: '@submitted', label: 'Submitted' }, ...columns, ...staffColumns]
  ...
  const valueOf = (r, key) => key === '@school' ? r.school : key === '@submitted' ? stamp(r.submittedAt) : (r.cells[key] ?? '')
  ...
  const header = ['Name', 'Email', ...shownCols.map(c => (c.earlier ? `${c.label} (earlier version)` : c.label)), ...(anyCorrected ? ['Corrections'] : [])]
  const line = (r) => [
    { v: r.name, f: styled(r.format?.['@name']) }, r.email,
    ...shownCols.map(c => ({ v: valueOf(r, c.key), f: styled(r.format?.[c.key]) })),
  ...
  const widths = [px('@name'), 0, ...shownCols.map(c => px(c.key)), ...(anyCorrected ? [0] : [])]
