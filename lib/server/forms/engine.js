...
export async function installStarters(db, profile) {
  ...
  for (const s of STARTER_FORMS) {
    ...
    const form = await createForm(db, { title: s.title, category: s.category, description: s.catalogDescription, starterKey: s.slug, slug, definition: s.definition }, profile)
