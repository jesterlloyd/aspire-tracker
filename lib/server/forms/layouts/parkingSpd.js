...
// boxes are theirs.
//
// PAPER-ORIGINAL-1 (2026-09-24, Owner: "use the exact same form and put the fields in the
// textbox"): when Parking Services' own PDF is on file for the form (uploaded from the builder
// into the private form-files bucket, never the repo), the answers are stamped into the boxes
// ON THAT FILE and nothing is redrawn. Only the exact file these positions were measured from
// is accepted (PARKING_SPD_SHA256); a different file would put answers beside the boxes.
//
...
import { answerText, formatDate } from '../../../../src/lib/forms/formModel.js'
...
/** SHA-256 of Parking Services' form as the Owner supplied it (Students Parking Data.pdf, 2025-02-07). */
export const PARKING_SPD_SHA256 = 'e199b82274c2c00798a48bfa0d8049c5018b1f0ea175e8f3f2cbbd543376e058'
 ...
 */
export function drawParkingSpd({ definition, answers, meta }, { doc, fonts, safe, wrap, logo, orgName, paper = false }) {
  const { regular, bold, italic } = fonts
  doc.setSubject('Students Parking Data (SPD)')
  // On the paper original, page one already carries every label, rule and box.
  const page = paper ? doc.getPage(0) : doc.addPage([612, H])
  ...
  // Without the paper original, the blank form is drawn first.
  if (!paper) drawBlank()
  function drawBlank() {
    ...
    for (const [, x, t, w, h, heavy] of BOXES) box(x, t, w, h, heavy)
    box(...YES_BOX); box(...NO_BOX)
  }

  // What the respondent answered, in the boxes.
  ...
  appendExtraAnswers({ definition, answers, placed: new Set([...BOXES.map(b => b[0]), 'parking_app']) }, { doc, fonts, wrap })
}
