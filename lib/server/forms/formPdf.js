...
import { fillScrubex } from './layouts/scrubex.js'
...
// A layout marked needsPaper has no redrawn copy: without its paper original, the plain PDF.
const LAYOUTS = { 'parking-spd': drawParkingSpd, scrubex: Object.assign(fillScrubex, { needsPaper: true }) }
 ...
 * paper: the layout's paper original (PDF bytes, already checked by the engine), or none.
 */
export async function buildSubmissionPdf({ definition, answers, who, meta, layout = null, logo = null, paper = null }) {
  // PAPER-ORIGINAL-1: with the paper original on file, the answers go onto that file.
  const draw = LAYOUTS[layout]?.needsPaper && !paper ? null : LAYOUTS[layout]
  const doc = draw && paper ? await PDFDocument.load(paper) : await PDFDocument.create()
  ...
  if (draw) {
    draw({ definition, answers, meta }, { doc, fonts: { regular, bold, italic }, safe, wrap, paper: !!paper,
      logo: paper ? null : await embedLogo(doc, logo), orgName: meta?.orgName })
