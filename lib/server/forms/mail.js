...
/**
 * FORM-FORWARD-1: a submitted form, sent to the office that acts on it, the respondent copied.
 * The PDF is attached; the email names who submitted it and says it was sent automatically.
 */
export function forwardEmail({ assignment, title, school = '' }) {
  const who = [assignment.name, school].filter(Boolean).join(', ')
  return {
    subject: `${title}: ${who}`,
    html: aspireEmailShell({ body: [
      P(`${assignment.name}${school ? ` (${school})` : ''} submitted the ${title}. Their completed form is attached.`),
      P(`It was sent automatically by ASPIRE Intelligence when they submitted it, and ${firstName(assignment.name)} is copied. Reply to reach ${assignment.sender_name || 'the ASPIRE team'}.`),
    ].join(''), preheader: `${title} from ${who}.` }),
  }
}
