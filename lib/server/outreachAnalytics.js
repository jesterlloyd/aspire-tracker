...
export function dayRange(startIso, endIso, tzOffsetMinutes = 0) {
  ...
  // but keep the RECENT edge of the window. Keeping the oldest 400 days made
  // current communications disappear while the KPI totals still counted them.
  const earliestVisible = new Date(last);
  earliestVisible.setUTCDate(earliestVisible.getUTCDate() - 399);
  const cur = requestedStart > earliestVisible ? requestedStart : earliestVisible;
  while (cur <= last) {
