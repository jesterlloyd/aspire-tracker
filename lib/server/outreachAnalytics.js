 ...
 */
export function aggregateOutreach(rows, { contactCategories = new Map(), startIso, endIso, tzOffsetMinutes = 0, observedRange = false } = {}) {
  ...
  const days = observedRange && observedDays.length
    ? dayRange(`${observedDays[0]}T00:00:00.000Z`, `${observedDays.at(-1)}T23:59:59.999Z`, 0)
    : (startIso && endIso) ? dayRange(startIso, endIso, tzOffsetMinutes) : observedDays;
