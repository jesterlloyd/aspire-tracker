...
// no-reply line. SYSTEM-DEFAULT-SIGNATURE-GIF-1: the signature now uses Jester's handwritten GIF
// (system-default policy) via aspireHandwrittenSignature; only the signature block changes.
// Token/URL/expiration/override behavior is unchanged.
...
import { SURVEY_NAMES } from '../../../src/lib/evaluation/surveyNames.js';

// Body-content accent color (the shared shell owns header/footer chrome). NAVY is used for the
// copy-paste fallback link; the survey-details card / CTA button / note now come from the primitives.
const NAVY = '#1D2567';
...
// Both the test and production builders render the same core block. `expiryText` differs slightly in
// wording between the two variants, so it is passed in (plain text now - rendered inside a Note).
//
// EMAIL-EVAL-MODERNIZE-3A: survey-details table → renderEmailDetailsCard; CTA → renderEmailButton with
// trustedUrl:true (the survey URL is a TOKENIZED server-generated link - it MUST pass through verbatim,
// escape-only, NEVER validated/normalized/hash-stripped); unique-link/expiry warning → renderEmailNote.
// The copy-paste fallback stays bespoke so the raw surveyUrl is preserved EXACTLY in href and text.
function surveyBodyCore({ timepointLabel, expiresAtHuman, surveyUrl, ctaLabel, expiryText }) {
  return `
${renderEmailDetailsCard({ title: 'Survey Details', rows: [
  { label: 'Instrument', value: SURVEY_NAMES.casey_fink_readiness_2024 },
  { label: 'Timepoint',  value: timepointLabel },
  { label: 'Expires',    value: expiresAtHuman },
] })}

<p style="margin:0 0 16px;">Please complete the survey using the secure link below:</p>

${renderEmailButton({ label: ctaLabel, url: surveyUrl, variant: 'navy', trustedUrl: true })}
...
${renderEmailNote({ body: expiryText, tone: 'info' })}`;
}
...
${surveyBodyCore({
    ...
    expiryText: `This link is unique. It will expire on ${expiresAtHuman}.`,
  })}
...
export function buildStudentInvitationEmail({
  ...
  const preheader = `Complete your ${SURVEY_NAMES.casey_fink_readiness_2024} survey as part of ASPIRE.`;
  const introHtml = renderBodyOverride(bodyOverride)
    || `<p style="margin:0 0 16px;">${escapeHtml(DEFAULT_INTRO)}</p>`;
...
${surveyBodyCore({
    ...
    expiryText: `This link is unique to you. Please do not share it. It will expire on ${expiresAtHuman}.`,
  })}
${aspireHandwrittenSignature('Kind regards,')}
`;
