...
import { reissueAssignment } from './assignmentReissue.js';
...
//   reissueRow       - SURVEY-REISSUE-2 (optional): this tuple's expired or revoked assignment
//                      row, already verified reissuable by the caller. When present the row is
//                      reused (token rotated, row re-activated) instead of a second row being
//                      inserted. Omitted by the manual PS-2b bulk send, which is unchanged.
export async function processPreceptorSend(ctx) {
  ...
  // SURVEY-REISSUE-2: the row being reissued may still carry a live status past its
  // expires_at. It is the row about to be reused, not a competing request.
  if (reissueRow) dupQuery = dupQuery.neq('id', reissueRow.id);
  ...
  // 4e. Create the assignment in a sendable state (status = 'sent'), or reissue this tuple's
  //     expired or revoked row (SURVEY-REISSUE-2). The token is minted first either way; the
  //     raw token lives only in this function scope.
  ...
  if (reissueRow) {
    const reissued = await reissueAssignment({
      db: supabaseAdmin, row: reissueRow, claimNote: `preceptor_progress:${period}:reissue_claim`,
      tokenHash, tokenHashPrefix, tokenExpiresAt, nowIso, logPrefix,
      activation: {
    ...
    if (!reissued.ok) {
      return {
        status: 'failed', student_id: studentId, student_name: studentName,
        classification: reissued.classification,
        reason: reissued.reason || reissued.error || 'Failed to reissue the feedback request',
      };
    }
    ...
    // 4f. Store the token hash against the new assignment.
    ...
    if (tokenErr) {
      ...
      if (rbErr) console.error(`${logPrefix} ROLLBACK FAILED, orphaned assignment:`, assignmentId, rbErr.message);
  ...
  if (sendError) {
    ...
    console.error(`${logPrefix} send_failed (assignment revoked):`, { assignment_id: assignmentId, error: sendError });
  ...
  try {
    ...
    console.error(`${logPrefix} log_write_failed:`, { assignment_id: assignmentId, error: logErr.message });
  }
  ...
  console.log(`${logPrefix} sent:`, {
    assignment_id: assignmentId, student_id: studentId, token_hash_prefix: tokenHashPrefix, period, source,
  });
  return {
    status: 'sent',
    assignment_id: assignmentId,
