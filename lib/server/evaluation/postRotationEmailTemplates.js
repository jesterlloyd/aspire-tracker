...
//
// Server-side email template builder for the Student's Feedback on ASPIRE workflow (slug:
// post_rotation_evaluation). Recipient is the STUDENT. This is NON-GATING experience feedback about
// the rotation, unit, and preceptor support. The email is focused only on feedback: it never
// mentions or attaches any award, and it references no other survey.
//
// Separate from the sibling evaluation email builders (readiness, preceptor, student experience),
// none of which are touched.
//
// Uses the shared ASPIRE system email shell + primitives, so header/footer/signature stay
// consistent and there is a single source of truth. studentFirstName stays HTML-escaped. The button
// URL is a tokenized server-generated link; trustedUrl:true preserves it verbatim (escape-only).
...
<p style="margin:0 0 16px;">Please use the button below to complete ${SURVEY_NAMES.post_rotation_evaluation}.</p>
