...
const NOTE_MAILTO_MAX = 254;   // RFC 5321 maximum address length
...
// data-mailto OPTIONAL (one address, linked inside the escaped body - never auto-detected).
...
function renderNote(attribs) {
  ...
  // Optional single mailto address. Capped here as policy; the primitive
  // validates the shape and refuses to link anything it does not recognize.
  const mailto = String(attribs['data-mailto'] || '').trim().slice(0, NOTE_MAILTO_MAX);
  return renderEmailNote({ title, body, tone: 'info', mailto });
}
