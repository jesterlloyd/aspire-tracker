...
export function buildDirectMessageEmail({ body, bodyFormat = 'text', includeSignature = true, signature = null }) {
  // SECURITY: the 'html' branch is the chokepoint for BOTH endpoints. Always sanitize here so raw
  // HTML can never reach the shell, even if a caller forgets to pre-sanitize (defense in depth).
  // 'text' behavior is unchanged: escape + pre-wrap.
