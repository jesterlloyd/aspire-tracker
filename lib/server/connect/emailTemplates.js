...
// ── Signature block ────────────────────────────────────────────────────────────
// CONNECT-COMMS-1D: renders the manual sender's signature. `sig` is a normalized object
// { displayName, credentials, title, affiliation, email, phone } resolved server-side; when
// null it falls back to the static Jester block (preserves pre-1D behavior). ALL fields are
// user-editable free text → escaped here (email also sits in a mailto: href attribute).
...
function signatureBlock(sig) {
  ...
  const titleLine   = s.title ? `${escapeHtml(s.title)}<br>` : ''
  const affiliation = escapeHtml(s.affiliation || 'Brawerman Nursing Institute, Cedars-Sinai')
  const email       = escapeHtml(s.email || '')
  const phonePart   = s.phone ? `&nbsp;|&nbsp;Office: ${escapeHtml(s.phone)}` : ''
  const emailLine   = email
    ? `<br><a href="mailto:${email}" style="color:${NAVY};">${email}</a>${phonePart}`
    : ''
  return `
...
<p style="margin:0;font-size:14px;color:${RAVEN};line-height:1.6;">
  <strong>${name}${creds}</strong><br>
  ${titleLine}${affiliation}${emailLine}
</p>`;
}
...
export function buildDirectMessageEmail({ body, bodyFormat = 'text', includeSignature = true, signature = null }) {
  ...
  // CONNECT-COMMS-1D: render the resolved per-user signature; null → static Jester fallback.
