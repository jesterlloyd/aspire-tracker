...
async function finalize(db, row, { outcome, resendId = null, errorCode = null, errorDetail = null }) {
  ...
  const { error } = await db.from('staff_notifications').update(update).eq('id', row.id)
  if (error) {
    throw new Error(`staff notification state update failed: ${sanitizeErrorText(error.message)}`)
  }
