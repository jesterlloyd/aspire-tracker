...
import { STUDENT_FILES_BUCKET, parseStoredFileRef } from '../studentFiles.js';
import { authorizeSkillForCaller, authorizeStudentResumeAccess, skillDeclaresData, denialMessage, DENY } from './skillAuthorization.js';
...
/**
 * Resolve a student by free-text name within the caller's active cohort.
 ...
 */
export async function resolveStudentByName(db, { name, cohortId }) {
  const q = String(name || '').trim();
  if (!q) return { ok: false, reason: DENY.STUDENT_NOT_FOUND };

  let query = db
    .from('students')
    .select('id, first_name, last_name, school, program_type, cohort_id, resume_url')
    .limit(25);
  ...
  const rows = (data || []).filter(s => {
    ...
    return full === needle || reversed === needle
      || full.includes(needle) || String(s.last_name || '').toLowerCase() === needle;
  });

  if (rows.length === 0) return { ok: false, reason: DENY.STUDENT_NOT_FOUND };
  if (rows.length > 1) {
    // Exact full-name match breaks a tie; otherwise ask rather than guess.
    const exact = rows.filter(s => `${s.first_name || ''} ${s.last_name || ''}`.trim().toLowerCase() === needle);
    if (exact.length !== 1) {
      return {
        ok: false,
        reason: DENY.STUDENT_AMBIGUOUS,
        candidates: rows.slice(0, 5).map(s => ({
          id: s.id, name: `${s.first_name} ${s.last_name}`, school: s.school || null,
        })),
      };
    }
    return { ok: true, student: exact[0] };
  }
...
export async function runResumeInterviewQuestions({
  ...
  if (studentId) {
    ...
    const resolved = await resolveStudentByName(db, { name: studentName, cohortId });
    if (!resolved.ok) {
