...
const HELP_INTENT = [
  ...
  /\bhow\s+does\s+.{0,60}?\bwork\b/i,
  /\bwhat\s+(?:does|do)\b.{0,60}?\bdo\b/i,
...
export function detectSkillHelp(userText, skills) {
  ...
  // 1. An explicit /slug reference is the clearest signal, and the one the
  //    reported defect used.
  const slashSlugs = extractSlashSlugs(text);
  for (const slug of slashSlugs) {
    if (bySlug.has(slug)) return { skill: bySlug.get(slug), ref: slug, matchedBy: 'slash_slug' };
  }
