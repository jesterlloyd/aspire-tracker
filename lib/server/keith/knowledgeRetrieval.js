...
import { isRoutingQuestion } from './queryIntent.js'
...
const EMAIL_ROUTING_ALIASES = [
  ...
  'Casey-Fink Readiness for Practice', 'Pre-Rotation', "Preceptor's Assessment of Student Readiness",
  "Student's Feedback on Unit and Preceptor", "Student's Feedback on ASPIRE",
...
// KEITH-GOVERNED-ROUTING-1: entries that ARE routing directories declare it
// themselves, in their own title or tags ("BNI … Routing Directory", tag
// "bni-routing"). No entry slug appears in code: any current or future entry
// that says it routes things qualifies, and un-declaring it in the vault
// removes it here automatically.
export function isRoutingDirectoryEntry(entry) {
  // "Routing Directory" in the title, or a routing tag. Deliberately NOT a bare
  // title match on "routing": the Email Routing Canon governs email CHANNELS,
  // not who-handles-what, and must not satisfy this seat.
  if (/\brouting\s+director/i.test(String(entry?.title || ''))) return true
  return (Array.isArray(entry?.tags) ? entry.tags : []).some(t => /routing/i.test(String(t)))
}

// For a WHERE-TO-DIRECT question ("who should I contact for…", "who handles…"),
// guarantee the best-scoring routing-directory entry a seat in the selection.
// The reported failure measured why: lexical scoring rewards surface-vocabulary
// overlap, so "…not an ASPIRE senior bedside preceptorship" outscored with the
// Program Overview entry (whose declared aliases are exactly those words) while
// the routing directory that actually answers the question sat at rank 16.
// A directory that merely QUALIFIES (>= MIN_SCORE) is appended - never replacing
// the lexical winners, and exempt from the block budget the way the top entry
// already is, because a routing question without its routing directory is the
// failure mode this exists to prevent.
function withRoutingDirectory(selected, scored, question) {
  if (!isRoutingQuestion(question)) return selected
  if (selected.some(isRoutingDirectoryEntry)) return selected
  const best = scored
    .filter(e => e.score >= MIN_SCORE && isRoutingDirectoryEntry(e))
    .sort((a, b) => b.score - a.score)[0]
  return best ? [...selected, best] : selected
}
...
export async function retrieveGovernedKnowledge(supabase, question) {
  ...
  try {
    ...
    const selected = withRoutingDirectory(selectEntries(scored), scored, question)
