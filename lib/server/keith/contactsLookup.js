...
// CONTACTS-CANON-1: category values are the canonical singular keys; rows are
// compared through canonicalizeCategory below so pre-migration plural values
// still match. Role lists carry canonical titles plus their legacy variants.
const ROLE_SIGNALS = [
  { kw: ['npd p', 'npdp', 'npd practitioner'], roles: ['unit npd p', 'unit npd practitioner', 'npd practitioner'] },
  { kw: ['preceptor', 'preceptors', 'cn ii', 'cn iii'], roles: ['preceptor', 'clinical preceptor', 'cn ii', 'cn iii'], category: 'Preceptor' },
  { kw: ['academic partner'], category: 'Academic Partner' },
  { kw: ['school coordinator', 'clinical placement coordinator', 'program coordinator', 'program assistant'], roles: ['school coordinator', 'clinical placement coordinator', 'program coordinator', 'program assistant'], category: 'Academic Partner' },
  { kw: ['unit leadership', 'unit leader'], category: 'Unit Leader' },
  { kw: ['associate director'], roles: ['associate director', 'interim associate director'] },
  { kw: ['assistant nurse manager', 'anm'], roles: ['assistant nurse manager'] },
  { kw: ['clinical nurse specialist', 'cns'], roles: ['clinical nurse specialist'] },
  { kw: ['nursing executive', 'chief nursing', 'executive director', 'nursing leadership'], category: 'Nursing Executive' },
  { kw: ['bni'], category: 'BNI Team' },
];

// Legacy plural stored values -> canonical (mirrors src/lib/contactCategories.js;
// kept local so this server module stays dependency-light).
...
function roleMatches(row, signals) {
  ...
  const cat = canonicalizeCategory(row.category);
