...
//
// Production QC caught pathways.md round-tripping one byte shorter: both
// halves trimmed the content, so a final newline (and any intentional
// trailing whitespace) was silently dropped. A reference is a FILE - it
// leaves through export as a file and may be diffed against its source, so
// "almost identical" is not identical.
//
// The separator is the whole trick. Compose writes EXACTLY "\n\n" between one
// reference's content and the next delimiter, so split removes exactly those
// two characters and never guesses. Content that itself ends in blank lines
// therefore survives: "abc\n\n" is stored as "abc\n\n" + "\n\n" + delimiter,
// and split hands back "abc\n\n". The last reference runs to end-of-string
// with no separator at all.
//
// Bodies written by the previous (trimming) version split correctly under
// this reader - the byte layout is identical, only the content it carried was
// pre-trimmed - so nothing stored needs migrating.
const REF_SEPARATOR = '\n\n'
...
export function composeInstructionBody(instructions, references = []) {
  let body = String(instructions || '').trim()
  for (const r of references) {
    // The reference's content is written verbatim: no trim, no normalization.
    body += `${REF_SEPARATOR}${REF_DELIM}${r.name} =====\n${String(r.content ?? '')}`
  }
  return body
}
...
export function splitInstructionBody(body) {
  ...
  for (let i = 0; i < marks.length; i++) {
    // Content begins after the delimiter line's own newline.
    const start = Math.min(marks[i].end + 1, text.length)
    const isLast = i + 1 === marks.length
    // Every reference but the last is followed by the separator compose wrote.
    const end = isLast ? text.length : marks[i + 1].start - REF_SEPARATOR.length
    references.push({ name: marks[i].name, content: text.slice(start, Math.max(start, end)) })
  }
