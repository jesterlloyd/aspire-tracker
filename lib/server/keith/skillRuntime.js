...
//   - slash_slug: the message names the skill by its own /slug, ANYWHERE in the
//     sentence. Typing a command's name is as deliberate as picking it from a
//     menu, and the composer only converts a LEADING /slug into skill_slug, so
//     "use /some-skill for this student" would otherwise reach nothing.
...
export function selectSkill(skills, { requestedSlug, userText }) {
  ...
  // An explicit /slug anywhere in the message. Registry-driven: a token is only
  // a command because the caller's own invocable list contains that slug, so
  // imported skills work identically and nothing is hardcoded.
  //
  // AMBIGUITY FAILS CLOSED. If the message names two DIFFERENT invocable
  // skills, this selects neither rather than guessing which one the caller
  // meant - there is no multi-skill execution contract to fall back on.
  const slugMatches = [];
  const seen = new Set();
  const slashRe = /\/([a-z0-9][a-z0-9._-]{1,80})/g;
  let m;
  while ((m = slashRe.exec(text)) !== null) {
    const slug = m[1];
    if (seen.has(slug)) continue;
    const hit = list.find(s => String(s.slug).toLowerCase() === slug);
    if (hit) { seen.add(slug); slugMatches.push(hit); }
  }
  if (slugMatches.length === 1) return { skill: slugMatches[0], mode: 'slash_slug' };
