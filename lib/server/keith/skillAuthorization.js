...
/**
 * Denial copy shown in chat.
 *
 * The entitlement sentence is now reachable ONLY from a genuine entitlement
 * failure. It used to be the `default:` branch, so every unhandled reason -
 * including "no such student" - told the caller to confirm their cohort
 * entitlement. An Owner, who is never entitlement-gated, was being advised to
 * ask an Owner for an entitlement they cannot need and would not fix. Copy that
 * names the wrong cause sends people to fix the wrong thing.
 *
 * Access denials still do not distinguish "you may not" from "no such student"
 * WITHIN the authorization gates, so a denied caller cannot probe for existence.
 * Resolution failures are a different matter: the caller already named the
 * student, so telling them the name did not match reveals nothing new.
 */
export function denialMessage(reason) {
  switch (reason) {
    ...
    case DENY.STUDENT_NOT_FOUND:
      return 'I could not find a student by that name in this cohort. Use the full name as it appears in ASPIRE and I will try again.';
    ...
    case DENY.DATA_GRANT_NOT_DECLARED:
      return 'That skill is not configured to read resumes, so I cannot use it here.';
    case DENY.ROLE_NOT_ALLOWED:
      return 'Your access level does not include this skill.';
    case DENY.NOT_ENTITLED:
    case DENY.ENTITLEMENT_LOOKUP_FAILED:
