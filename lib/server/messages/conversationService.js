...
export function buildGeneralTeamPayloadFingerprint({ actorKind, subject, category, body, schoolKey = null } = {}) {
  const payload = {
    version: 1,
  ...
  // Academic Partner: the SERVER-VERIFIED canonical school_key is part of the idempotency identity, so
  // the same actor + request_id + subject + body + delivery under a DIFFERENT authorized school
  // produces a DIFFERENT fingerprint and hits the ledger's "different payload" conflict (MS409) instead
  // of replaying the original school's result. Omitted for student/unit_leader, so their fingerprint is
  // byte-identical to before (no school context).
...
// ── Portal: reply ───────────────────────────────────────────────────────────
// MESSAGES-CORRECTNESS-PHASE0-1: the reply carries the VERIFIED caller actor kind
// (student, unit_leader, or academic_partner) so messages_post_reply persists the
// true author_role, instead of the previous hardcoded 'student'. Authorization is
// unchanged and server-derived: actorKind comes from verifyPortalMessagesCaller,
// never from the request body, and the RPC re-authorizes via
// message_participant_can_send regardless of the kind.
//
// PRE-MIGRATION BRIDGE (remove after the Owner applies
// 20260730000001_messages_phase0_correctness.sql): the LIVE messages_post_reply
// rejects 'academic_partner' outright and forces a unit_leader's expected event to
// 'unit_leader_message', so a non-student kind with a portal_reply delivery fails
// MS400 until the migration lands. On exactly that signature we retry ONCE with
// the legacy 'student' kind, which reproduces today's shipped behavior exactly.
// Post-migration the first call succeeds and the retry path is dead code.
export async function replyForPortal(deps, { profile, actorKind = 'student', conversationId, conversation, body }) {
  const { delivery, reason } = routeSingle({
  ...
  const payload = buildDeliveryPayload({
    ...
    conversationId,
    delivery,
    senderName: profile.full_name || 'ASPIRE portal user',
    subject: conversation?.subject || 'ASPIRE Messages',
    category: conversation?.category || null,
  });

  const callRpc = (kind) => deps.db.rpc('messages_post_reply', {
    p_actor_profile_id: profile.id,
    p_actor_kind: kind,
  ...
  let { data, error } = await callRpc(actorKind);
  if (error && actorKind !== 'student' && String(error.code) === 'MS400') {
    // Pre-migration bridge (see header). MS400 for a non-student portal kind can
    // only mean the live RPC predates the Phase 0 migration; body-length MS400s
    // would fail identically under either kind, so one retry is safe and cheap.
    console.warn('[messages] messages_post_reply pre-Phase0 fallback to student kind', { actorKind });
    ;({ data, error } = await callRpc('student'));
  }
  if (error) return { ok: false, rpcError: error };
...
export async function replyForPortalDirect(
  ...
  const payload = buildDeliveryPayload({
    ...
    conversationId,
    delivery,
    senderName: profile.full_name || (actorKind === 'unit_leader' ? 'Unit Leader' : actorKind === 'academic_partner' ? 'Academic Partner' : actorKind === 'nursing_academic' ? 'Nursing Education & Leadership' : 'ASPIRE student'),
    subject: conversation?.subject || 'ASPIRE Messages',
    category: conversation?.category || null,
  });
...
export async function startGeneralTeamConversationForPortal(
  ...
  const payloadFingerprint = buildGeneralTeamPayloadFingerprint({
    ...
    category,
    body,
    // Academic Partner only (null for student/unit_leader): binds the fingerprint to the verified school.
    schoolKey,
  });
  ...
  // Academic Partner uses the dedicated AP RPC (server-verified selected school_key);
  // NA-PORTAL-UTILITIES-1: Nursing Education & Leadership uses its own dedicated RPC.
  ...
  const { data, error } = actorKind === 'academic_partner'
    ...
    ? await deps.db.rpc('messages_start_general_team_conversation_na', {
      p_actor_profile_id: profile.id,
      p_request_id: requestId,
      p_payload_fingerprint: payloadFingerprint,
      p_subject: subject,
