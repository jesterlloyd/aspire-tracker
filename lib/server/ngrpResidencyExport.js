...
import {
  effectiveEligibility, rosterStatusLabel, preferencesOf, effectivePreferences, NOT_PROCEEDING_REASONS,
} from './ngrpPool.js'
...
const BASE_COLUMNS = [
  ...
  // RESIDENCY-ROSTER-1: the roster's own Status column, so the file and the
  // screen say the same thing. "Application" is gone from both: confirmation is
  // automatic now, and Status carries the whole arc.
  ['Status', ({ candidate }) => (candidate ? rosterStatusLabel(candidate) : 'Not in Pool')],
  ['Not Proceeding Reason', ({ candidate }) =>
    (candidate?.not_proceeding_reason ? NOT_PROCEEDING_REASONS[candidate.not_proceeding_reason]?.label || '' : '')],
  ['Not Proceeding Note', ({ candidate }) => candidate?.not_proceeding_note],
  // The choices in force, which is the staff ranking when one has been set.
  // The alumnus's own submitted ranking is still below, under the form answers.
  ['Choice 1', ({ candidate }) => preferencesOf(candidate)[0]],
  ['Choice 2', ({ candidate }) => preferencesOf(candidate)[1]],
  ['Choice 3', ({ candidate }) => preferencesOf(candidate)[2]],
  ['Choices Set By', ({ candidate }) => (effectivePreferences(candidate).source === 'staff' ? 'ASPIRE team' : 'Alumnus')],
  ['Paired Unit (Interviewing)', ({ candidate }) => candidate?.assigned_unit],
