...
import { validateSessionDetails } from '../../src/lib/ngrp/ngrpMentorshipSession.js'
...
export function validateSupportEntry(input, { today } = {}) {
  ...
  // MENTORSHIP-1: a mentorship session is a full record (format, duration,
  // topics, next steps). logged_by is decided here, never taken from the
  // request: this endpoint is the ASPIRE team's. Every other activity keeps its
  // exact shape, so recording Before Residency needs no new columns.
  let session = null
  if (src.activity === 'mentorship_session') {
    const s = validateSessionDetails(src, { loggedBy: 'aspire_team' })
    if (s.ok) session = s.details
    else errors.push(...s.errors)
  }
