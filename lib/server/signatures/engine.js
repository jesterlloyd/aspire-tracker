...
import {
  ...
  maskEmail, initialsOf, colorForIndex, envelopeCode, SENDER_ROLE, isAutoField, autoFieldValue,
...
async function sendMail(mailer, msg) {
  const r = await sendMailResult(mailer, msg)
  return r.ok ? (r.data || { id: null }) : null
}

/** The same send, keeping the reason a refusal gave, for the audit trail. */
async function sendMailResult(mailer, { from, to, subject, html, replyTo, attachments }) {
  try {
    const { data, error } = await mailer.emails.send({ from, to: [to], subject, html, ...(replyTo ? { reply_to: replyTo } : {}), ...(attachments ? { attachments } : {}) })
    return error ? { ok: false, error: String(error.message || error.name || 'refused').slice(0, 200) } : { ok: true, data }
  } catch (e) { return { ok: false, error: String(e?.message || 'send failed').slice(0, 200) } }
}
...
export async function finishSigning(db, { signer, request, values, adopted, ctx, appUrl, mailer, settings, method = 'email_code', sealOptions = {} }) {
  ...
  for (const f of mine) {
    if (isAutoField(f)) merged[f.id] = autoFieldValue(f.type, now, settings.time_zone)
...
async function sendCopies(db, { request, signers, bytes, mailer }) {
  const recipients = signers.filter(s => s.status !== 'replaced').map(s => ({ name: s.name, email: s.email, id: s.id, role: 'signer' }))
  const sender = String(request.sender_email || '').trim()
  if (sender) {
    const same = recipients.find(r => r.email.toLowerCase() === sender.toLowerCase())
    if (same) same.role = 'signer and sender'
    else recipients.push({ name: request.sender_name, email: sender, id: null, role: 'sender' })
  }
  // Every recipient's outcome is recorded, so the trail never claims a copy went out that
  // the mail service refused. "Accepted" means the mail service took it, not that it arrived.
  const results = []
  for (const r of recipients) {
    const mail = completedEmail({ request, recipientName: r.name, sealedSha256: request.sealed_sha256 })
    const sent = await sendMailResult(mailer, { from: fromLine(request.sender_name), to: r.email, ...mail,
      attachments: [{ filename: `${request.title.replace(/[^\w .-]+/g, '').slice(0, 80) || 'Signed document'} (sealed).pdf`, content: bytes.toString('base64') }] })
    results.push({ to: maskEmail(r.email), role: r.role, ok: sent.ok, ...(sent.ok ? {} : { error: sent.error }) })
    if (sent.ok && r.id) await db.from('sig_request_signers').update({ completed_copy_sent_at: new Date().toISOString() }).eq('id', r.id)
  }
  await appendEvent(db, { requestId: request.id, type: 'copies_sent', actor: 'System',
    details: { recipients: recipients.length, accepted: results.filter(x => x.ok).length, sender_included: !!sender, results } })
}
