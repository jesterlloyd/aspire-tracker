 ...
 * keepPaths (FORM-SHEET-1): keep "/" in names, for a package like .xlsx whose parts live in
 * folders. Off by default, so a download of signed copies still flattens every name.
 */
export function zipStored(files, when = new Date(), { keepPaths = false } = {}) {
  const locals = [], centrals = []
  let offset = 0
  const seen = new Set()
  for (const f of files) {
    let name = keepPaths ? f.name.replace(/[\\:*?"<>|]+/g, '_').replace(/^\/+/, '') : f.name.replace(/[\\/:*?"<>|]+/g, '_')
