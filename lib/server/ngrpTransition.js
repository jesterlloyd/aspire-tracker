...
//
// DELIVERY-SAFE TOKEN LIFECYCLE (the correction): a resend PREPARES the
// replacement token as 'pending' - unusable by the public endpoint - while
// the OLD token stays active. Only after the provider ACCEPTED the email does
// ngrp_activate_token_tx atomically activate the new token, revoke the old
// one, and (for a first send) flip the pending assignment to 'sent'. A
// provider failure runs ngrp_fail_token_tx instead: the pending token becomes
// 'failed', the old link keeps working, and a never-delivered first-send
// assignment is revoked. Nothing can ever read as Sent before acceptance,
// and a failed resend can never strand the alumnus without a working link.
...
// ctx: { db, cycle, student, candidate|null, assignment|null, actorProfileId,
//        generateToken, sendEmail, buildEmail, baseUrl, batchId? }
...
//           assignmentId?, tokenHashPrefix?, providerAccepted? } - never the
// raw token. 'sent'/'resent' means the provider ACCEPTED the email AND the
// new token was activated; every other path is 'failed' and leaves any
// previously working link untouched.
export async function sendOneTransitionForm(ctx) {
  const { db, cycle, student, actorProfileId, generateToken, sendEmail, buildEmail, baseUrl, batchId } = ctx
  ...
  // 2. Assignment: reuse the live one (a delivered one keeps its lifecycle
  ...
  if (!assignment) {
    ...
    if (live.error) return { outcome: 'failed', reason: 'assignment_read_failed', candidateId: candidate.id }
    assignment = live.assignment
  }
  if (!assignment) {
    const inserted = await db.from('ngrp_transition_assignments')
      .insert({ candidate_id: candidate.id, sent_by_profile_id: actorProfileId, status: 'pending', sent_at: null })
      .select('*').maybeSingle()
    if (!inserted.data) return { outcome: 'failed', reason: 'assignment_write_failed', candidateId: candidate.id }
    assignment = inserted.data
  }

  // 3. Prepare the REPLACEMENT token as 'pending'. Any old active token
  //    stays valid: nothing is revoked until the new email is accepted.
  ...
  if (!insertedToken.data) return { outcome: 'failed', reason: 'token_write_failed', candidateId: candidate.id }
  const tokenId = insertedToken.data.id

  // 4. Send. The URL carries the raw token in the FRAGMENT only. The
  //    provider idempotency key (batch + candidate) makes a process retry
  //    after acceptance a no-op at the provider.
  ...
  const sent = await sendEmail({
    ...
    idempotencyKey: batchId ? `ngrp-transition/${batchId}:${candidate.id}` : undefined,
  })

  if (!sent.ok) {
    // Delivery failed: the pending token becomes 'failed' and a
    // never-delivered first-send assignment is revoked - atomically, and
    // WITHOUT touching any old active token, which keeps working.
    await db.rpc('ngrp_fail_token_tx', {
      p_token_id: tokenId, p_actor: actorProfileId, p_reason: sent.reason || 'provider_failed',
    })
    return { outcome: 'failed', reason: sent.reason || 'provider_failed', candidateId: candidate.id }
  }

  // 5. Provider accepted: atomically activate the new token, revoke the old
  //    one, and flip a first send's pending assignment to 'sent'.
  const act = await db.rpc('ngrp_activate_token_tx', { p_token_id: tokenId, p_actor: actorProfileId })
  if (act.error) {
    // The email is out but its link was never activated: fail the pending
    // token (dead either way) and report honestly - NOT sent. The old link,
    // if any, is still active; a retry issues a fresh working link.
    await db.rpc('ngrp_fail_token_tx', {
      p_token_id: tokenId, p_actor: actorProfileId, p_reason: 'activation_failed',
    })
    return { outcome: 'failed', reason: 'activation_failed', candidateId: candidate.id, providerAccepted: true }
  }
