...
import { getEligibleUnits } from '../../src/lib/unitCatalog.js'
...
const PRECEPTED_UNIT_CHOICES = new Set([...getEligibleUnits().map(u => u.name), 'Other'])
...
export function validateSubmission(rawPayload, { activeUnitNames = [], checklist = null, rules = null, requireComplete = true } = {}) {
  ...
  const canonical = {
    ...
    aspire: {
      ...
      // Picked from the ASPIRE placement-unit catalog, or 'Other' with the unit
      // named in precepted_unit_other. Anything else is dropped, so the answers
      // stay comparable across alumni even if the page is bypassed.
      precepted_unit: PRECEPTED_UNIT_CHOICES.has(str(aspire.precepted_unit, 120)) ? str(aspire.precepted_unit, 120) : '',
      precepted_unit_other: str(aspire.precepted_unit, 120) === 'Other' ? str(aspire.precepted_unit_other, 120) : '',
      rotation_shifts: numOrNull(aspire.rotation_shifts),
    ...
    attestation: {
      ...
      // Required: HR sees every submitted form, so a submission without
      // this consent could never be shown to them.
      consent_hr_share: att.consent_hr_share === true,
    },
  }
  ...
  if (requireComplete) {
    ...
    // ASPIRE experience: optional, but an 'Other' unit must be named and a
    // shift count must be a whole number.
    if (canonical.aspire.precepted_unit === 'Other' && !canonical.aspire.precepted_unit_other) {
      push('aspire.precepted_unit_other', 'Name the unit where you precepted.')
    }
    const shifts = canonical.aspire.rotation_shifts
    if (shifts !== null && (!Number.isInteger(shifts) || shifts < 0)) {
      push('aspire.rotation_shifts', 'Enter your completed shifts as a whole number.')
    }
    ...
    if (!canonical.attestation.consent_hr_share) push('attestation.consent_hr_share', 'Consent to share your responses with Cedars-Sinai Talent Acquisition is required.')
