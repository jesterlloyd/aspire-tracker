...
export const ASPIRE_NOREPLY_LINE =
  'Please do not reply to this automated email. For questions, email us at aspire@cshs.org.';

function orgValue(organization, key, fallback = '') {
  return String(organization?.[key] || fallback)
}

export function applyOrganizationBranding(html, organization = {}) {
  const email = orgValue(organization, 'general_email', 'aspire@cshs.org')
  const name = orgValue(organization, 'display_name', 'Cedars-Sinai')
  ...
  return String(html || '')
    .replaceAll('jesterlloyd.bautista@cshs.org', email)
    .replaceAll('JesterLloyd.Bautista@cshs.org', email)
    .replaceAll('Email Jester at', 'Email us at')
    .replaceAll('email Jester directly at', 'email us at')
    .replaceAll('Cedars-Sinai Medical Center &bull; 8700 Beverly Blvd, Los Angeles, CA 90048', `${name}${address ? ` &bull; ${address}` : ''}`)
}
...
export function aspireEmailShell({ body = '', preheader = '', footerNote = ASPIRE_NOREPLY_LINE, organization } = {}) {
  const orgName = orgValue(organization, 'display_name', 'Cedars-Sinai')
  const alt = orgValue(organization, 'logo_alt_text', `${orgName} logo`)
  const logo = orgValue(organization, 'header_logo_url', orgValue(organization, 'document_logo_url', LOGO_URL))
  const mark = orgValue(organization, 'footer_logo_url', MARK_URL)
  const address = [organization?.address_line_1, organization?.address_line_2, organization?.city, organization?.state_province, organization?.postal_code].filter(Boolean).join(', ')
  const contact = organization?.general_email ? `Please do not reply to this automated email. For questions, email us at ${organization.general_email}.` : footerNote
  return `<!DOCTYPE html>
...
<tr><td style="background:${NIGHTFALL};padding:14px 28px;">
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%"><tr>
    <td align="left" style="vertical-align:middle;">
      <img src="${logo}" alt="${alt}" width="140" height="62"
...
<tr><td style="background:${NIGHTFALL};padding:18px 28px;">
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%"><tr>
    <td width="64" style="vertical-align:top;padding-right:12px;">
      <img src="${mark}" alt="${alt}" width="48"
           style="display:block;width:48px;max-width:48px;height:auto;border:0;" />
    </td>
    <td style="vertical-align:top;">
      <p style="margin:0;color:#ffffff;font-size:12px;font-weight:700;line-height:1.5;">${orgName} &bull; ASPIRE Intelligence</p>
      <p style="margin:3px 0 0;color:rgba(255,255,255,0.82);font-size:11px;line-height:1.5;">${address}</p>
      <p style="margin:12px 0 0;color:rgba(255,255,255,0.6);font-size:10px;line-height:1.55;">${contact}</p>
    </td>
  </tr></table>
</td></tr>
