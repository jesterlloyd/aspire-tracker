...
// A single, ordinary email address. Deliberately strict: anything else is not
// linked at all rather than guessed at.
const NOTE_MAILTO_RE = /^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/;

export function renderEmailNote({ title = '', body = '', tone = 'info', mailto = '' } = {}) {
  if (!body) return '';
  ...
  // PRECEPTOR-ATTACHMENT-REMINDER-1: an OPT-IN mailto link, and the strictest
  // possible version of one. The body is escaped FIRST, then the one address
  // the caller named - which must be a well-formed address AND must already
  // appear in the body - is swapped for an anchor whose href and text are that
  // same address. Nothing is auto-detected, no caller-supplied HTML is echoed,
  // and callers that pass no `mailto` render byte-identically to before.
  const addr = String(mailto || '').trim();
  if (addr && NOTE_MAILTO_RE.test(addr) && body.includes(addr)) {
    const shown = escapeHtml(addr);
    bodyHtml = bodyHtml.split(shown).join(
      `<a href="mailto:${escapeAttr(addr)}" style="color:${NIGHTFALL};text-decoration:underline;">${shown}</a>`
    );
  }
