...
const ROLE_COPY = {
  ...
  nursing_academic: {
    subject: 'You’re invited to the ASPIRE Nursing Education & Leadership Portal',
    portalName: 'ASPIRE Nursing Education & Leadership Portal',
    intro: 'your organization-wide view of ASPIRE at Cedars-Sinai. From the portal you can see school rotation dates and fiscal-year impact, review and download the aggregate community-benefit report, and view the active ASPIRE contact directory.',
  },
  talent_acquisition: {
    subject: 'You’re invited to the ASPIRE Residency Portal',
    portalName: 'ASPIRE Residency Portal',
    intro: 'your shared view, with the ASPIRE team, of ASPIRE alumni moving into the Cedars-Sinai New Graduate RN Residency Program. From the portal you can review submitted Transition Forms, eligibility, and ranked unit preferences, record interviews and hires, and manage each residency cohort’s settings.',
  },
}
...
export function portalInvitationEmail({ firstName, activationLink, expiresAt, role } = {}) {
  ...
  // activation deadline.
  // The sentence itself lives in lib/server/activationLifetime.js so this
  // template, the staff template and both activation-page states state one
  // duration. That module also carries the provenance of the figure and the
  // configuration history; it is descriptive copy only, and the runtime
  // authority remains the Supabase Auth setting, not this repository.
  const linkLifetimeLine = `<p style="margin:0 0 16px;font-size:13px;color:#6b7280;">${ACTIVATION_LIFETIME_SENTENCE}</p>`
