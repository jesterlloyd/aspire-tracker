...
// expiration (3600 seconds) - a Supabase Auth project setting (mailer_otp_exp)
// that no code in this repository controls, so the sentence must be
// re-verified against the dashboard before it is ever changed. It is worded
// identically to the portal invitation on purpose: one rule, one sentence. Staff access itself has no expiration, so this
