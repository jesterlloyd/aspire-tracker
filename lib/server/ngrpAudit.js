...
export const NGRP_AUDIT_EVENTS = Object.freeze([
  ...
  // RESIDENCY-ROSTER-1. Mirrored by the DB CHECK widened in 20260916000000.
  'not_proceeding_recorded', 'application_reinstated', 'unit_preferences_set', 'offer_declined', 'not_selected',
  // RESIDENCY-REFLECTION-1. Mirrored by the DB CHECK widened in 20260917000000.
  'reflection_started', 'reflection_sent', 'reflection_opened', 'reflection_submitted', 'reflection_stopped',
])
...
const SAFE_META_KEYS = new Set([
  ...
  // A removal reason, a period number and a period count are labels, not
  // personal data; nothing the alumnus wrote travels with them.
  'reason', 'corrected', 'previous_reason', 'preferences', 'cleared', 'period_number', 'period_count',
])
