...
async function readCandidates(db, cycleId) {
  const full = await db.from('ngrp_candidates').select(`${CANDIDATE_FIELDS}, ${PLACEMENT_FIELDS}`).eq('cycle_id', cycleId)
  if (!full.error || !isMissingNgrpColumn(full.error)) return full
  return db.from('ngrp_candidates').select(CANDIDATE_FIELDS).eq('cycle_id', cycleId)
}
...
async function readOutcomes(db, candidateIds) {
  const full = await db.from('ngrp_residency_outcomes').select(`${OUTCOME_FIELDS}, cs_email`).in('candidate_id', candidateIds)
  if (!full.error || !isMissingNgrpColumn(full.error)) return full
  return db.from('ngrp_residency_outcomes').select(OUTCOME_FIELDS).in('candidate_id', candidateIds)
}
