...
import { NOT_PROCEEDING_REASON_KEYS, REASON_REQUIRING_NOTE, normalizePreferences } from './ngrpPool.js'
import { RESIDENT_SHIFTS } from '../../src/lib/ngrp/ngrpReflectionForm.js'
...
//
// RESIDENCY-ROSTER-1 (Owner, 2026-09-12): pairing an applicant with a unit on
// the placement board means that unit will INTERVIEW them. Four results can
// follow, and until now only two of them were recordable. not_selected_at and
// offer_declined_at are the other two, so "we interviewed them and it did not
// work out" stops being an absence of data.
export function validateOutcomePayload(input) {
  ...
  for (const [key, field] of [
    ...
    ['hired_at', 'hired_at'],
    ['not_selected_at', 'not_selected_at'],
    ['offer_declined_at', 'offer_declined_at'],
  ]) {
  ...
  // RESIDENCY-REFLECTION-2 (Owner, 2026-09-14): the shift they were hired into,
  // Rotation's own vocabulary. It colours their schedule marks; Variable means
  // they name the shift per day.
  const shift = typeof src.shift === 'string' ? src.shift.trim() : ''
  if (!shift) out.shift = null
  else if (RESIDENT_SHIFTS.includes(shift)) out.shift = shift
  else errors.push({ field: 'shift', message: 'Choose Day, Night, Mid or Variable.' })

  // Where residency correspondence goes once they are hired (Owner,
  // 2026-09-11). Optional: the account often does not exist on day one, and
  // until it does the personal email is the backup
  // (lib/server/ngrpResidencyRecipient.js).
  const cs = typeof src.cs_email === 'string' ? src.cs_email.trim() : ''
  if (!cs) out.cs_email = null
  else if (cs.length <= 200 && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cs)) out.cs_email = cs
  else errors.push({ field: 'cs_email', message: 'Enter a valid Cedars-Sinai email address.' })
  ...
  // One final result per attempt. Hired, Declined Offer and Not Selected are
  // three answers to the same question, and a record holding two of them would
  // make the roster's Status column a coin toss. The DB enforces this as well.
  const finals = [
    ['hired_at', 'Hired'], ['offer_declined_at', 'Declined Offer'], ['not_selected_at', 'Not Selected'],
  ].filter(([k]) => out[k])
  if (finals.length > 1) {
    errors.push({
      field: finals[finals.length - 1][0],
      message: `This alumnus cannot be both ${finals[0][1]} and ${finals[1][1]}. Clear one of them.`,
    })
  }
  if (out.offer_declined_at && !out.offer_extended_at) {
    errors.push({ field: 'offer_declined_at', message: 'Record the offer before recording that it was declined.' })
  }
  if (out.not_selected_at && out.offer_accepted_at) {
    errors.push({ field: 'not_selected_at', message: 'An accepted offer is on record, so Not Selected cannot also be true.' })
  }
  if (errors.length) return { ok: false, errors }
...
// ── RESIDENCY-ROSTER-1 ──────────────────────────────────────────────────────
//
// Recording that an alumnus is NOT PROCEEDING: they leave the Applicant Pool
// and stop counting as an applicant, while the submitted form and the
// eligibility result stay exactly where they are. The reason is required, from
// the one allowlist, because "withdrawn" with no reason answers nothing six
// months later. The DB enforces both rules too.
export function validateNotProceedingPayload(input) {
  const src = (input && typeof input === 'object') ? input : {}
  const errors = []
  const reason = typeof src.reason === 'string' ? src.reason : ''
  if (!NOT_PROCEEDING_REASON_KEYS.includes(reason)) {
    errors.push({ field: 'reason', message: 'Choose why this alumnus is not proceeding.' })
  }
  const note = typeof src.note === 'string' ? src.note.trim() : ''
  if (reason === REASON_REQUIRING_NOTE && !note) {
    errors.push({ field: 'note', message: 'Say what happened, since the reason is Other.' })
  }
  if (note.length > 2000) errors.push({ field: 'note', message: 'Keep the note to 2000 characters or fewer.' })
  if (errors.length) return { ok: false, errors }
  return { ok: true, reason, note: note || null }
}

// A staff ranking of unit choices, replacing the one the alumnus submitted.
// An empty list is a legitimate save: it clears the staff ranking and the
// submitted one becomes effective again.
export function validateUnitPreferencesPayload(input) {
  const src = (input && typeof input === 'object') ? input : {}
  if (src.preferences !== null && src.preferences !== undefined && !Array.isArray(src.preferences)) {
    return { ok: false, errors: [{ field: 'preferences', message: 'Send the ranked choices as a list.' }] }
  }
  const preferences = normalizePreferences(src.preferences || [])
  if ((src.preferences || []).some(p => typeof p === 'string' && p.length > 120)) {
    return { ok: false, errors: [{ field: 'preferences', message: 'A unit name must be 120 characters or fewer.' }] }
  }
