...
| 20260928000000_forms_phase3.sql | FORMS-PHASE3 (ASPIRE Catalog Phase 3), branch forms-phase3, 2026-09-24 | **NOT APPLIED (drafted 2026-09-24).** Additive: catalog_forms (draft, settings, current version), catalog_form_versions (published versions, frozen by trigger: UPDATE refused), form_assignments (one person's copy: link hash, due date, sent/opened/submitted/closed/voided, reminders), form_submissions (answers, filed PDF), and the private 'form-files' bucket (10 MB). Every table has org_id and an Owner/Admin read policy; writes are service role only. Requires 20260927000000 (organizations, sig_caller_org_id) and 20260926000000 (kind 'form', record source 'form_submission'). Applying it adds empty tables; the Catalog shows Forms once they exist, and the starter forms are added from + New afterwards. Checks: PRE 1 to POST 5 of db/audit/forms_phase3_checks.sql (test/formsEngine.test.mjs runs that file against Postgres). Rollback at the end of the migration. |
