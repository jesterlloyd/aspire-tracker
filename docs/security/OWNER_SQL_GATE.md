...
| 20260930000000_s15_unit_leader_thread_read_scope.sql | S15-1 (FINDINGS_REGISTER S-15), 2026-09-24 | UNKNOWN (drafted 2026-09-24; NOT APPLIED). Redefines `message_participant_can_read` so the unit_leader branch requires an active `user_unit_scopes` row for the thread's unit (any active unit scope for a general row), the same test `message_participant_can_send` already applies. One function, one transaction, grants restated; no table or row is touched, `removed_at` is never written. Run PRE 1 to 5 of `db/audit/s15_unit_leader_thread_read_scope_checks.sql`, then the file as ONE block, then POST 1 to 6. STOP conditions are stated in each section. Rollback: re-apply the can_read block from 20260828000000 (instructions at the end of the migration). |
