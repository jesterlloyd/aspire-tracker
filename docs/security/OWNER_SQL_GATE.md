...
| 20261004000000_s04_interview_tables_write_split.sql | S04-1 (FINDINGS_REGISTER S-04, last part), 2026-09-25 | UNKNOWN (drafted 2026-09-25; NOT APPLIED). Replaces the three Wave E FOR ALL `is_staff()` policies on `interview_availability_blocks`, `interview_slots` and `interview_sessions` with the split every other core table has: SELECT on `is_staff()`, INSERT, UPDATE, DELETE on `is_active_staff_writer()`. Only the three named policies and its own names are dropped; any other policy survives. One transaction, safe to re-run. **Apply only after S04-1 is live in production**: interviewer self-service (pause a block, block or unblock a slot, mark a Teams invite) runs through `api/availability.js` from that commit; before it those are browser writes this file would silently refuse. Run PRE 1 to 3 of `db/audit/s04_interview_tables_write_split_checks.sql`, then the file as ONE block, then POST 1 to 4. Rollback at the end of the migration. |
