...
| 20261001000000_s16_avatar_writes_server_only.sql | S16-1 (FINDINGS_REGISTER S-16), 2026-09-24 | UNKNOWN (drafted 2026-09-24; NOT APPLIED). Removes the database-side avatar write paths a browser session could still use once the application no longer does: revokes the `avatar_url` column UPDATE from authenticated on `user_profiles` (the other five self-service columns keep theirs), revokes browser EXECUTE on `update_my_avatar`, drops the two `contact-avatars` write policies by name and any write policy naming the `avatars` bucket by expression (each raised as a NOTICE). Read policies stay; no row is touched. One transaction. **Apply only after S16-1 is live**, or the old uploaders fail on the revoked grant. Run PRE 1 to 4 of `db/audit/s16_avatar_writes_server_only_checks.sql` (keep PRE 3), then the file as ONE block, then POST 1 to 4. Rollback at the end of the migration. |
