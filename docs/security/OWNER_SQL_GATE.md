...
| 20261002000000_s08_school_form_password_hash.sql | S08-1 (FINDINGS_REGISTER S-08, part two), 2026-09-25 | UNKNOWN (drafted 2026-09-25; NOT APPLIED). Step A of two. Installs pgcrypto if absent, creates `cohort_form_secrets` (RLS on, no browser policy), drops and recreates `school_form_requires_password` and `verify_school_form_password` from the repository (same signatures, SECURITY DEFINER, pinned search_path, EXECUTE to anon, authenticated, service_role), adds service_role-only `set_school_form_password`, backfills a bcrypt hash for every cohort with a plaintext password and proves each verifies before committing. The plaintext column is KEPT here. One transaction; safe before or after S08-1 is live. Run PRE-A 1 to 4 of `db/audit/s08_school_form_password_hash_checks.sql` (STOP if PRE-A 1's bodies differ from the recorded TRIM-equality semantics), then the file as ONE block, then POST-A 1 to 5. Rollback at the end of the file. |
| 20261003000000_s08_school_form_password_plaintext_drop.sql | S08-1 (FINDINGS_REGISTER S-08, part two), 2026-09-25 | UNKNOWN (drafted 2026-09-25; NOT APPLIED). Step B of two. **Only after** 20261002000000 is APPLIED with POST-A 1 to 5 green **and** S08-1 is live in production (after this file a write to `cohorts.school_form_password` fails, which the old modal code did on every save). Re-proves every plaintext verifies through its hash, NULLs and drops the column, recreates the three functions without the fallback. Run PRE-B 1 to 2, then the file as ONE block, then POST-B 1 to 4 (POST-B 4 is the live try on the school form, by hand). Not reversible: the plaintext values are gone by design. |
