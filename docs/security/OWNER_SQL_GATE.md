...
| 20261007000000_s24_cohort_school_rotations_read_scope.sql | S24-1 (FINDINGS_REGISTER S-24), 2026-09-25 | **APPLIED 2026-09-26 by the Owner.** Apply: "Success. No rows returned." PRE 1: exactly `cohort_school_rotations_anon_select` ({anon}) and `cohort_school_rotations_authenticated_select` ({authenticated}), both SELECT, qual true. PRE 2: `is_staff` security_definer true, rls_enabled true, anon_select true, authenticated_select true, service_select true. PRE 3: total_rows 14, staff_profiles 4, portal_profiles 8. POST 1: 1 row, `cohort_school_rotations_staff_select`, SELECT, {authenticated}, qual `is_staff()`. POST 2: anon_select false, anon_any false, authenticated_select true, service_select true, rls_enabled true. POST 3: "S-24 POST 3 PASS (rolled back on purpose): anon=refused authenticated_no_jwt=0 portal_user=0 staff_user=14 total_rows=14". Drops `cohort_school_rotations_anon_select` and `cohort_school_rotations_authenticated_select` (both USING (true), from 20260522000000), creates `cohort_school_rotations_staff_select` FOR SELECT TO authenticated USING (`is_staff()`), and revokes the anon role's table grant. Every browser reader is the staff app; every portal and public read goes through a service-role endpoint that scopes its own result; no anonymous page reads the table. Writes untouched (service role only). One transaction, safe to re-run, refuses without the table or `is_staff()`. **No deploy dependency**: no application code changes with it. Run PRE 1 to 3 of `db/audit/s24_cohort_school_rotations_read_scope_checks.sql`, then the file as ONE block, then POST 1 to 3 (POST 3 ends with a deliberate RAISE EXCEPTION whose message is the report). Rollback at the end of the migration. |
| 20261008000000_s28_s29_constraints_and_activity_logs.sql | S28-1 / S29-1 (FINDINGS_REGISTER S-28, S-29, S-23 follow-up), 2026-09-26 | UNKNOWN (drafted 2026-09-26; NOT APPLIED). Adds `uq_eval_tokens_one_active` (partial unique on `evaluation_assignment_tokens (assignment_id) WHERE revoked_at IS NULL AND used_at IS NULL`), `uq_interview_sessions_one_per_slot` (partial unique on `interview_sessions (slot_id) WHERE slot_id IS NOT NULL`), and the S-23 trigger pair plus the UPDATE/DELETE/TRUNCATE revoke on `activity_logs` through `public.append_only_refuse()` (needs 20261006000000, applied). Each index is behind a guard that names violating ids and refuses the whole file. One transaction, safe to re-run. **Deploy dependency: apply only after S28-2 is live** (the same commit; `cancel_booking` now clears `slot_id` on a kept session, without which a rebooking of a cancelled slot would be refused). Run PRE 1 to 5 of `db/audit/s28_s29_constraints_and_activity_logs_checks.sql` (PRE 2 and PRE 3 must return no rows), then the file as ONE block, then POST 1 to 5 (POST 5 ends with a deliberate RAISE EXCEPTION whose message is the report). Rollback at the end of the migration. |
