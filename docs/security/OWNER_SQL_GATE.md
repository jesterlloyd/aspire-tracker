...
| 20260927000000_signatures_phase2.sql | SIGNATURES-PHASE2 (ASPIRE Catalog Phase 2), branch signatures-phase2, 2026-09-23 | **NOT APPLIED (drafted 2026-09-23).** Additive: organizations, feature_flags (catalog.signatures created 'off'), sig_settings (seal provider name, RFC 3161 TSA URL, no secrets), sig_disclosures (v1.0, DRAFT for Legal), sig_templates, sig_bulk_sends, sig_requests, sig_request_signers, sig_field_values, sig_events (hash-chained by trigger; UPDATE, DELETE and TRUNCATE refused), and the private 'signature-documents' bucket (PDF, 25 MB). Every new table has org_id and a read-only Owner/Admin (or staff) policy; writes are service role only. Applying it turns NOTHING on. Checks: PRE 1 to POST 7 of db/audit/signatures_phase2_checks.sql (test/signaturesMigration.test.mjs runs that file against Postgres). PRE 1 must be all false: a pre-existing organizations or feature_flags table is a STOP. Turning the flag on is a separate Owner write listed at the end of the checks file. Rollback at the end of the migration (loses every request and its audit trail). |
