...
test('new general conversation uses a stable request id and the final endpoint contract only', () => {
  ...
  assert.match(client, /const payload = \{ request_id: requestId, body \}/)
...
test('shared bubbles render ASPIRE Team incoming left and portal user outgoing right', () => {
  ...
  assert.match(globalCss, /\.msg-bubble-incoming \{[\s\S]*background: var\(--messages-bubble-in,[\s\S]*color: var\(--messages-bubble-in-ink,/)
  assert.match(globalCss, /\.msg-bubble-outgoing \{[\s\S]*background: var\(--messages-bubble-out,[\s\S]*color: var\(--messages-bubble-out-ink,/)
