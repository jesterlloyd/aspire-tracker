...
test('matched corner behavior and accessibility are explicit', () => {
  ...
  assert.match(layer, /hidden=\{utilitiesHidden\}/)
  assert.match(layer, /messagesEnabled && !utilitiesHidden && \(/)
  ...
  assert.match(sharedFeedback, /announceFloatingPanelOpen\('feedback'\)/)
  assert.match(sharedFeedback, /onFloatingPanelOpen/)
  assert.match(sharedFeedback, /event\.pointerType === 'mouse'/)
  assert.match(app, /<MainMessagesLauncher portalPreview \/>/)
