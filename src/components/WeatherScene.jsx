...
import { sceneForTime, sunTimesFrom, artSceneFor, ALL_SCENES, isNightScene, isWetCode } from '../lib/mastheadScene'
...
export function useMastheadScene() {
  ...
  // MASTHEAD-CLOUDY-1: whether anything is FALLING. The cloudy-night frame
  // serves both a dry overcast night and a storm; rain and lightning run only
  // when the weather is wet. QA override: aspire_wet_override_v1 = '1' | '0'.
  let wet = isWetCode(data?.code)
  try {
    const w = localStorage.getItem('aspire_wet_override_v1')
    if (w === '1' || w === '0') wet = w === '1'
  } catch { /* storage unavailable: the weather wins */ }
  // Night treatment follows every night scene, clear or clouded.
  return { scene, night: isNightScene(scene), wet }
}
