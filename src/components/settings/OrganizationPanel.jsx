...
function LogoCard({ title, description, dark, url, alt, onFile, onRemove }) {
  return <div className="org-logo-card">
    <div className={`org-logo-preview ${dark ? 'dark' : 'light'}`}>
      {url ? <img src={url} alt={alt || ''} /> : <div className="org-logo-fallback"><Building2 size={22} /><span>{dark ? 'Header logo' : title}</span></div>}
    </div>
...
export default function OrganizationPanel() {
  ...
  const [files, setFiles] = useState({ header: null, document: null, footer: null })
  const [removed, setRemoved] = useState({ header: false, document: false, footer: false })
  ...
  const save = async event => { event.preventDefault(); setBusy(true); setNotice(null); const result = await call('POST', { ...form, header_logo: files.header, document_logo: files.document, footer_logo: files.footer, header_logo_remove: removed.header, document_logo_remove: removed.document, footer_logo_remove: removed.footer }); setBusy(false); if (!result.ok) { setNotice({ ok: false, text: result.data?.fields ? Object.values(result.data.fields).join(' ') : (result.data?.message || 'Could not save organization settings.') }); return } setSaved(result.data.organization); setFiles({ header: null, document: null, footer: null }); setRemoved({ header: false, document: false, footer: false }); setNotice({ ok: true, text: 'Organization settings saved.' }) }
  const cancel = () => { setForm(Object.fromEntries(TEXT_FIELDS.map(([field]) => [field, saved?.[field] || '']))); setFiles({ header: null, document: null, footer: null }); setRemoved({ header: false, document: false, footer: false }); setNotice(null) }
  ...
  const footerUrl = removed.footer ? null : (files.footer ? `data:${files.footer.type};base64,${files.footer.data_base64}` : saved?.footer_logo_url)
  return <section className="org-panel" aria-labelledby="organization-settings-title"><SettingsPageHeader id="organization-settings-title" title="Organization" subtitle="Manage the identity and contact information used across ASPIRE Intelligence." /><form onSubmit={save}>
    <div className="org-layout"><div className="org-form-stack">
      <SurfaceCard className="org-card"><div className="org-card-heading"><h2>Identity &amp; Branding</h2><p>These values appear wherever your organization is represented.</p></div><div className="org-fields">{TEXT_FIELDS.slice(0, 4).map(([field, label, required]) => <label key={field} className={field === 'logo_alt_text' ? 'full' : ''}><span>{label}{required && <i>Required</i>}</span><input value={form[field]} required={required} onChange={event => setField(field, event.target.value)} /></label>)}</div><div className="org-logo-grid"><LogoCard title="Header logo" description="Used on Nightfall application and email headers." dark url={headerUrl} alt={form.logo_alt_text} onFile={choose('header')} onRemove={() => setRemoved(previous => ({ ...previous, header: true }))} /><LogoCard title="Document logo" description="Used on light document surfaces and PDFs." url={documentUrl} alt={form.logo_alt_text} onFile={choose('document')} onRemove={() => setRemoved(previous => ({ ...previous, document: true }))} /><LogoCard title="Footer logo" description="Used in Nightfall email footers." dark url={footerUrl} alt={form.logo_alt_text} onFile={choose('footer')} onRemove={() => setRemoved(previous => ({ ...previous, footer: true }))} /></div></SurfaceCard>
      ...
      <UsageCard />
    </div><LivePreview form={form} headerUrl={headerUrl} documentUrl={documentUrl} footerUrl={footerUrl} /></div>
