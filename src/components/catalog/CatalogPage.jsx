...
import { Search, FileText, FileType2, ExternalLink, Star, Pin, Folder, Clock } from 'lucide-react'
...
export default function CatalogPage() {
  ...
  // Open: internal_file → server signed URL; external_link → navigate out.
  const openResource = useCallback(async (r) => {
    setOpenError(null)
    if (r.resource_type === 'external_link') {
      if (r.external_url) window.open(r.external_url, '_blank', 'noopener,noreferrer')
      return
    }
    // internal_file — open a blank tab synchronously (user gesture), then point it at the
    // short-lived signed URL once the server returns it. The URL is used immediately and
    ...
    try {
      ...
      if (res.ok && body.signedUrl) {
        ...
        if (pending) pending.close()
        setOpenError(body.error ? `Could not open “${r.title}”: ${body.error}` : `Could not open “${r.title}”.`)
      }
    } catch {
      if (pending) pending.close()
      setOpenError('Network error opening file. Please try again.')
    } finally {
      setOpeningId(null)
    }
  }, [])
  ...
  return (
    <div style={{ padding: '4px 24px 40px', maxWidth: 1280, margin: '0 auto', fontFamily: F }}>
      ...
      <div className="stat-cards-row" style={{ marginBottom: 24 }}>
        <div className="summary-card card-nightfall">
          <div className="summary-card-value">{metrics.resources}</div>
          <div className="summary-card-label">Resources</div>
        </div>
        <div className="summary-card card-marina">
          <div className="summary-card-value">{metrics.categories}</div>
          <div className="summary-card-label">Categories</div>
        </div>
        <div className="summary-card card-green">
          <div className="summary-card-value">{metrics.recent}</div>
          <div className="summary-card-label">Recently Updated</div>
          <div className="summary-card-sub">Last 30 days</div>
        </div>
        <div className="summary-card card-amber">
          <div className="summary-card-value">{metrics.featured}</div>
          <div className="summary-card-label">Featured</div>
        </div>
      </div>
                ...
                return (
                  <div key={r.id} style={{
                    ...
                    <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      >
                        {external ? <>Open external <ExternalLink size={13} strokeWidth={2} /></> : (busy ? 'Opening…' : 'Open')}
                      </button>
                      {/* Overflow placeholder — not wired in CATALOG-1 */}
                      <span aria-hidden="true" style={{ color: '#cbd0d6', fontSize: 18, fontWeight: 700, lineHeight: 1, padding: '0 4px', userSelect: 'none' }}>⋯</span>
                    </div>
                  </div>
                )
              })}
