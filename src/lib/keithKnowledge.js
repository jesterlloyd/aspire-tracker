...
- Reveal decided_by_name, decision_origin, or internal notes at the individual student level`,
  ...
  tabs: {
    ...
    rotation: `The Rotation tab has three sub-tabs: Placement Board, Preceptors, and Activity (Activity is for staff who can edit). The Placement Board is the matching board. At the top, a titleless KPI band reads Students, Placed, Unplaced, Open Slots, and Top Choice. Below it the board is two columns: Students on the left (a cream leather column of white paper notes) and Units on the right (a column of pastel unit boards). A student note carries their avatar, name, school, ASPIRE Status pill, shift chip, an availability pill only when it warrants a look, and their top three units stacked down the right side, with "(Full)" after a choice that has no open slot. The pool is alphabetical by last name with the not-yet-interviewed at the bottom. Clicking a unit board regroups the pool under labels ("Picked 5 SCCT as 1st choice", "Picked as 2nd or 3rd choice", "All other students" dimmed); clicking a student ribbons their 1st, 2nd and 3rd choice boards and brings them to the front. A placement is made by dragging a note onto a board or by clicking the student and then the board; a green (+) appears under the cursor only over a board with an open slot. A placed note is pinned, and the pin's colour and number say which choice it matched (1 green, 2 amber, 3 navy, a dot for a placement outside their top three). Pulling the pin unmatches the student: there is no confirmation dialog, and a ten-second Undo in the toast is the safeguard. The board's own filters are a School dropdown on the Students column and a Division dropdown on the Units column; there is no search and no sort. Placed students without a preceptor show "+ Assign preceptor" on the board, and a full unit with unnotified placements shows "Notify Unit Leader". The Preceptors sub-tab is the preceptor roster (Add Preceptor, search, and a table of name, email, unit, shift type, active status, current student, cohort count, last active date). Residency has the same board under its own nouns: Residency > Interview Board pairs Interviewees with Hiring Units, and unpairing there is immediate with an Undo that simply pairs them again.`,
  },
  ...
  csLinkWorkflow: `CS-Link Access is a two-stage process. Stage 1 is a Service Center request: new students need Add Non-Employee; former students and current Cedars-Sinai employees or volunteers need Update Non-Employee or Reactivate Non-Employee, and staff then confirm the account is active. Stage 2 is adding CS-Link access for all students. The Action Center flags students from Form Received onwards who have not yet had Stage 1 submitted.`,
...
- Rotation (R): Placement matching workspace; its Placement Board matches students to units, Students on the left and Units on the right.
...
- StudentMatchingCard -- the paper note in the Placement Board's Students column
- UnitCard (EmbedUnitCard) -- a unit board in the Placement Board's Units column
