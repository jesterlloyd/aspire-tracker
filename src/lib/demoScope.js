...
/**
 * Every table the boundary filters. A table appears here ONLY once its is_demo column
 * exists in the database, because filtering on a column that is not there turns every
 * read of that table into a 400 and takes the app down.
 *
 * Phase 1 covers the spine: the tables that hold people, and the tables behind the
 * stages a demo actually walks through (interview, placement board, rotation, on
 * campus, evaluation). Later phases extend it. Keep this list and the migration in
 * lockstep; test/demoScope.test.mjs fails when they drift.
 */
